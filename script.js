...
async function api() {
    ...
    for (let i = 1; i < data.length; i += 3) {
        try {
            ...
            let code22 = data[i + 1].codeAlpha2
            ...
            let code33 = data[i + 2].codeAlpha2;
...
btn.setAttribute('onclick','empty()')
...
btn1.setAttribute('onclick','empty()')
...
async function holiday(e) {
    ...
    let code = parent.getAttribute('code1');
    console.log(code)
...
function empty(){
    let close = document.querySelector('#staticBackdrop .modal-body') // to get the modal-body to empty the data
   close.innerHTML=" ";
